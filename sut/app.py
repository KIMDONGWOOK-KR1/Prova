...
표의 위치 표시다.
    ...
    화면        결함  내용                                            FAIL 기대
    login       B1    비밀번호 규칙(길이·대문자·특수문자) 검증 없음    ┐ 6건 (B1 이
                B2    이메일 형식 검증 없음                            │  규칙 셋에서
                B3    기획서와 다른 에러 문구                          │  각각 잡힘)
                B4    안내 문구(placeholder)가 기획서와 다름           ┘
...
## bad 로그인에 심은 의도적 불일치 4종

    B1  비밀번호 규칙(8자 이상·대문자·특수문자) 검증 미구현
        → "abcd1234"로도 통과됨. 기획서가 요구한 규칙이 구현에 없는 전형적 사례.
          검사를 통째로 건너뛰므로 규칙 셋이 각각 FAIL 한다.
    B2  이메일 형식 검증 누락
        → "not-an-email"도 통과됨.
    B3  미등록 계정 에러 문구 불일치
        → 기획서 "이메일 또는 비밀번호가 올바르지 않습니다."
           구현   "로그인 정보를 확인해주세요."
    B4  이메일 입력란 안내 문구 불일치 (templates/login.html — C4 처럼 마크업 동일 원칙의
        예외다. 안내 문구 자체가 검증 대상이라 달라야 잡힌다)
        → 기획서 "이메일을 입력하세요", 구현 "아이디를 입력하세요"
