...
## 검색 화면을 붙이기 전에 적어 둔 것 — 그리고 실제로 어땠나

이 노트를 쓸 때 "코드를 열기 전에 검토하면 절반은 미리 찾는다"는 교훈에 따라 네 가지를
적어 뒀습니다. 검색 화면을 실제로 붙인 뒤 대조해 보면 이렇습니다.

| 미리 적어 둔 것 | 실제로는 |
|---|---|
| **검증 축이 다르다.** `Expectation`에 새 유형이 필요할 것 | 맞았지만 **새 유형은 필요 없었다.** 기획서가 건수를 "검색 결과 3건" 문구로 노출한다고 적어 두므로 기존 `text_visible`로 충분했다. 문구로 확인하는 편이 실물 마크업에 덜 의존한다 |
| **위반값이라는 개념이 흐려진다.** `rule_expander`의 모델에 맞는지 봐야 함 | **맞지 않았다.** 규칙과 별개 경로(`ScreenSpec.scenarios`)를 새로 뒀다. 억지로 규칙으로 표현하지 않은 것이 옳았다 |
| **SUT에 데이터가 필요하다.** 테스트 간 상태 공유를 생각해야 함 | **문제가 되지 않았다.** 검색은 읽기 전용이라 상태를 바꾸지 않는다. `conftest.py`의 세션 범위 픽스처 근거가 오히려 강해졌다 |
| **완료 조건은 같다.** 로그인·회원가입 결과 유지 | 맞았다. 유지됐다 |

절반은 맞고 절반은 틀렸습니다. **틀린 쪽이 더 유용했습니다** — 예상보다 적게 만들면 된다는
것을 알았고, 예상하지 못한 곳(S1의 표 읽기)에서 진짜 문제가 나왔습니다.

그리고 미리 적지 못한 것이 하나 있었습니다. 이번 확장에서 가장 값진 교훈인데,
**표에 그대로 적혀 있는 사실을 AI에게 읽어 달라고 부탁하고 있었다**는 것입니다.
같은 종류의 실패를 네 번 겪고서야 알았습니다 —
[09-search-scenarios.md](09-search-scenarios.md)의 "가장 값진 교훈"에 있습니다.

**배운 것**: 사전 검토는 값이 있지만 정답표가 아닙니다. 예측을 적어 두는 진짜 이득은
'맞히는 것'이 아니라 **끝난 뒤 대조해서 무엇을 못 보는지 아는 것**입니다.

---

다음: [09-search-scenarios.md](09-search-scenarios.md) — 규칙으로 표현할 수 없는 검증
