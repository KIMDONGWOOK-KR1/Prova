...
| **필요한 게 조회다** | **필요한 게 해석이다** |

마지막 두 줄은 나중에 배운 것입니다.

`same_as`에서 배운 것: "요소 하나만 봐선 안 된다"가 곧 "AI가 필요하다"는 뜻은 아닙니다.
필요했던 것은 추론이 아니라 **순서**였습니다.

검색 화면에서 배운 것은 더 아팠습니다. **표에 그대로 적혀 있는 값**을 AI에게 읽어 달라고
부탁하고 있었고, 틀릴 때마다 프롬프트를 고쳐 달래려 했습니다. 같은 종류의 실패를 네 번
겪고서야 코드로 옮겼습니다.

```
"8자 이상"          -> min_length: 8        ← 자연어 해석,  AI
"비밀번호와 동일"     -> same_as: password    ← 자연어 해석,  AI
표의 3열 값          -> 라벨                 ← 조회,        코드
표의 행 수           -> 요소 개수             ← 세기,        코드
```

자세한 내용은 [09-search-scenarios.md](09-search-scenarios.md)의 "가장 값진 교훈"에
있습니다. 요약하면 — **표 안에 구조로 적혀 있지 않은 것만 AI가 판단합니다.**
