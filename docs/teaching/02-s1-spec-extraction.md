...
실제로 확인했습니다 — `tests/test_s1_golden.py`가 세 화면(로그인·회원가입·검색)에서
**54/54 통과**했습니다. Claude API가 필요 없습니다.

다만 여기까지 오는 데 함정이 넷 있었고, 그걸 겪고서 **1단계가 할 수 있는 일이 처음
생각보다 많다**는 것을 알게 됐습니다. 이 노트 끝의 "결론" 절이 그 이야기입니다.
...
함정 3과 4를 겪고도 프롬프트만 고쳤습니다. 검색 화면을 추가하면서 **같은 종류가 두 번
더** 나왔습니다.

| 무엇을 | 7B가 한 일 |
|---|---|
| 요소 목록 | 7행 표에서 버튼 행을 생략 (함정 3) |
| `sample_value` | few-shot 표 제목을 바꾸자 두 번 놓침 (함정 4) |
| `screen_id` | `화면 ID \| search`를 두고 `product_search`를 지어냄 |
| `scenarios` | 없는 시나리오를 창작, 실패 조건 표의 행을 가져옴 |

네 번째까지 오고서야 공통점이 보였습니다. **넷 다 표에 그대로 적혀 있는 값입니다.**
`pdfplumber`가 이미 괘선으로 열을 갈라 읽고 있는데, 그걸 다시 AI에게 "읽어 줘"라고
부탁하고 있었던 것입니다.

```python
# pdf_parser.py — AI 를 쓰지 않는다
doc.declared_element_ids()    # 요소 ID 목록
doc.declared_screen_meta()    # 화면 ID / 경로
doc.declared_sample_values()  # 라벨 -> 예시값
doc.declared_scenarios()      # 입력-결과 짝
```

앞의 셋은 프롬프트에 주입하고(예방) 결과를 대조합니다(검출). **마지막 것은 결과를
대체합니다** — 보정이 아니라 애초에 AI가 판단할 일이 아니었기 때문입니다.

표를 고르는 기준이 **캡션이 아니라 열 제목**이라는 점이 이 방식을 견고하게 만듭니다.
...
**그래서 이 노트의 "왜 두 단계로 나눴나"가 더 중요해졌습니다.** 1단계(결정적 추출)가
할 수 있는 일이 처음 생각보다 많습니다. AI에 남는 일은 **표 안에 구조로 적혀 있지 않은
것** 뿐입니다 — "8자 이상"을 `min_length: 8`로 옮기는 것 같은 자연어 해석.

자세한 내용은 [09-search-scenarios.md](09-search-scenarios.md)에 있습니다.
