...
### 도구와 대상을 같은 사람이 만들면 대상이 도구에 맞춰진다

8월부터 검증한 대상은 전부 우리 SUT 였다. 공개 연습 사이트 세 곳에 처음 돌리자 실패가
**전부** '기획서와 다름' 으로 떴는데, 사이트는 멀쩡했다. 폼 안의 같은 이름 버튼, 아이콘
글리프가 붙은 버튼 이름, 조작 전부터 화면에 있는 기대 문구 — 셋 다 우리 SUT 에는 없던
모양이다. 도구가 못 다루는 모양은 애초에 SUT 에 들어가지 않았던 것이다.

통과도 믿을 수 없었다. 하나는 조작과 무관하게 참인 문구로, 하나는 로그인하지 않고
'에러 없음' 으로 통과했다. 오탐만 세면 이 둘은 보이지 않는다.

→ 드러난 모양을 `set_content`·`page.route` 로 떼어 테스트에 고정했다(외부 사이트는
바뀐다). 확인하지 못한 것은 새 분류 '확인 불가' 와 '약한 확인' 으로 따로 센다(설계 판단
19~21). **실물 기획서를 기다리는 동안에도 실물 대상은 먼저 잴 수 있다** — 이번이 그랬다.


### 픽스처를 늘리니 쓰이지 않던 경로와 못 쓰는 경로가 함께 드러났다

2026-09-23 에 화면 넷(문의하기·비밀번호 변경·배송지 등록·방문 예약)을 더하면서,
**`rule_expander` 가 지원하는데 아무 기획서도 쓰지 않던 규칙**을 일부러 골랐다 —
`pattern`·`require_lowercase`·`require_digit`. 셋 다 처음 케이스가 만들어졌고
심은 결함을 정확히 짚었다. 코드에 있고 아무도 쓰지 않는 기능은 동작한다는 증거가
없다는 것이 이 저장소가 반복해서 치른 값인데, 그 증거를 픽스처로 만든 셈이다.

같은 방식으로 **못 쓰는 경로**도 드러났다. 방문 예약의 방문 날짜를 `date` 유형
필수로 적었더니 정상 케이스가 `good` 에서 FAIL 했다. 스텝 목록에 날짜를 채우는
스텝이 **아예 없었다.**

원인은 `models.py` 가 이미 적어 둔 설계다 — date 요소는 값을 자동으로 채우지
않는다(`input[type=date]` 는 ISO 날짜만 받아 자유 문자열 견본값이 통하지 않는다).
값은 주문조회의 필터 케이스가 시드 표에서 계산해 넣는다.

그 결과가 이것이다. **필수 날짜 요소가 있는 기획서는 지금 도구로 통과할 수 없다.**
정상 케이스가 날짜를 비운 채 제출해 "필수 입력 항목입니다." 를 받는다. 설계로 적혀
있었지만 그 설계가 **어떤 기획서를 못 받는지**는 아무도 쓰지 않아 드러나지 않았다.

→ 이 픽스처는 선택 목록으로 돌아갔다(예약 가능한 날짜가 정해져 있다는 설정은
자연스럽다). 실물 기획서는 그렇게 비켜 가 주지 않는다 — 필수 날짜는 흔하다.
`_fillable_inputs` 에 date 를 넣고 `sample_value` 를 ISO 날짜로 채우는 길이 있지만,
주문조회의 필터 케이스와 값이 겹치는지 먼저 봐야 한다. 로드맵에 남긴다.

### PDF 줄바꿈이 값 안에서 일어나면 파서가 공백을 끼워 읽는다

문의하기의 안내 문구를 `010-0000-0000` 으로 두었더니 판정이 이렇게 났다.

    문구가 다름 — '연락처': 기대 '010-0000-000 0', 화면 '010-0000-0000'

기획서 표의 칸이 좁아 PDF 가 값 **안에서** 줄을 바꿨고, 결정적 파서가 그 줄바꿈을
공백으로 읽었다. 화면도 기획서도 맞는데 도구만 틀린, 순수한 오탐이다.

픽스처는 긴 숫자열을 피해 넘어갔다. 그러나 **실물 기획서에서 다시 만날 자리다** —
로드맵 최우선 항목('실물 기획서로 S1 확인')이 찾으려던 종류의 어긋남이고, 우리가
상상한 훼손 6종에는 없던 모양이다.
