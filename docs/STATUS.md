...
> **2026-09-23 오후에 닫힌 것** — 7B '모델 메모' 잡음(main `c2c76ff`·`fc853b9`),
> colspan 빈 통과(설계 판단 22), 리포트 완결성(지표 5), 판정 정확도(지표 2),
> 웹 UI 의 잘못된 막힘 안내. 표에서 뺐다.
...
| 2 | **PASS/FAIL 판단 정확도** | ≥ 90% | **196/196 = 100.0%** · 오탐 0 · 미탐 0 · 커버리지 100% | `docs/measurements/verdict-accuracy-2026-09-23.md` · `fixtures/labels/verdict-labels.json` |
| 5 | **리포트 완결성** | **100%** | **793/793 = 100.0%** (리포트 12개·케이스 196건). 첫 측정은 79.1% 였다 — 입력 데이터가 0/166 | `docs/measurements/report-completeness-2026-09-23.md` |
...
> **지표 2·5 는 2026-09-23 에 닫혔다.** 남은 지표 6 은 외부 API 키가 필요하고,
...
> 원래 이 자리에 있던 두 항목은 **2026-09-23 에 끝났다** — 리포트 완결성 체크리스트
> (지표 5 = 100%)와 주문조회 정답 라벨(지표 2 = 100%, 커버리지 100%).

- **왜 좋은가** — Python 코드가 아니라 YAML 이다. 망가뜨릴 코드가 없다. 그리고 이제
  **돌릴 것이 셋**이다: `pytest`, `eval_report_completeness.py`, `eval_verdict_accuracy.py`.
  뒤의 둘은 목표 미달이면 종료 코드 1 을 내도록 이미 만들어 두었으니 그대로 걸면 된다
- **어디에** — `.github/workflows/test.yml` (지금 `.github/` 자체가 없다)
- **주의** — 실모델 테스트 105개는 GPU 가 없으면 자동 skip 되므로 CI 에서도 그대로 skip 된다.
  두 측정 스크립트는 SUT 를 띄우고 `prova run` 을 12번 돌려야 리포트가 생긴다 (약 2분)
- **예상** 0.5~1일
...
- **잊지 말 것** — 결함을 새로 심으면 `fixtures/labels/verdict-labels.json` 의 라벨도
  같이 고친다. 안 고치면 채점기가 `ValueError` 로 멈춰 알려 준다
- **본보기** — `docs/teaching/20-date-filter.md` 와 설계 판단 15
- **예상** 2~3일

### ③ 머리글 없는 표에 남은 빈 통과 (지표 2 가 못 잡는 자리)

- **왜 좋은가** — 오늘 막은 것과 **같은 모양의 마지막 조각**이다. colspan 은 막았지만
  (설계 판단 22), 머리글이 아예 없는 표는 여전히 `absent` → '0건으로 확인' PASS 가 난다.
  재현 방법과 고치는 방향이 설계 판단 22 에 그대로 적혀 있다
- **왜 오늘 안 했나** — 라벨이 화면에 정말 없으니 `absent` 가 거짓이 아니다. 탐지 자체를
  넓혀야 닫히고, 그러면 조건부 렌더링 경로(기존 테스트가 지키는 것)와 부딪힌다
- **어디에** — `s3_grounder/dom_locator.py` · `sut/templates/`
- **본보기** — `tests/test_table_unverifiable.py` 가 경계를 지키는 방식 그대로
- **예상** 2~4일
...
## 5. 웹 UI (`prova serve`) — 계획서 산출물 6번

```powershell
uv run prova serve     # http://127.0.0.1:7007
```

계획서 2-2 의 산출물 6번 "QA Agent 웹 데모/프로토타입 — 설계 문서 업로드 → 자동
테스트 → 결과 확인" 이 이것이다. **외부 자원을 하나도 불러오지 않는다** — CDN 폰트도
아이콘도 없다. 리포트가 폐쇄망에서 열려야 한다는 원칙과 같다.

| 파일 | 줄 수 | 하는 일 |
|---|---:|---|
| `src/prova/server/app.py` | 409 | 라우트 9개. `/api/plan`(브라우저 없음) · `/api/run`(승인 목록) · `/api/runs` · `/api/upload` |
| `src/prova/server/runner.py` | 125 | 워커 스레드 하나. **동시 실행 하나** — 브라우저·GPU 를 둘이 쓰면 판정 타이밍이 흔들린다 |
| `static/index.html` | 119 | 사이드바(설정) + 메인(무대) 뼈대 |
| `static/app.js` | 766 | 화면 전체. 계획 → 승인 → 실행 → 결과 |
| `static/app.css` | 663 | 색은 `theme.py` 의 `TOKENS_CSS` 하나에서 온다 (`/static/tokens.css`) |

### 실행을 두 단계로 자르는 것이 이 화면의 핵심이다

`계획 만들기` 는 **브라우저를 열지 않는다**(S1~S2 만). 무엇을 테스트할지 먼저 보여
주고, 사람이 케이스를 끄거나 요청을 고친 뒤 `이대로 실행` 을 누른다. 자연어 요청이
결함을 덮지 못하게 하는 장치 중 하나다(설계 판단 14, 노트 18).

### 2026-09-23 에 고친 것

처음 쓰는 사람의 경로를 따라가며 셋을 고쳤다.

- **GPU 없이 첫 클릭** — 기본 백엔드가 `vllm` 이라 연결 실패로 끝나는데, 다음에 할
  행동이 없었다. 답('연습용')이 바로 위 드롭다운에 있는데 화면이 말하지 않았다.
  이제 연결 실패 사유 뒤에 안내가 붙는다. **mock 으로 대신 돌려 주지는 않는다.**
- **409 를 한 문구로 덮던 것** — 서버는 '이미 실행 중' 과 '대상 웹앱이 안 떠 있다'
  를 구분해 보내는데 화면이 둘 다 "이미 실행 중입니다" 로 그렸다. SUT 를 안 띄운
  사람이 **있지도 않은 작업을 기다렸다.**
- **`static` 캐시** — `app.js` 를 고쳐도 브라우저가 옛 것을 써서 화면이 안 바뀐다.
  이걸 모르면 코드를 의심하며 같은 자리를 두 번 고친다. 이제 `no-store` 다.

### 아직 없는 것

| 남은 것 | 난이도 | 메모 |
|---|---|---|
| 2차 경로(VLM)를 웹 UI 에서 못 켠다 | 중 | CLI 는 `--vlm URL` 로 켠다. `configs/default.yaml` 의 `vlm_confidence_threshold` 주석이 "웹 UI 는 아직 VLM 을 붙이지 않아 이 값을 읽어도 아무 일도 하지 않는다" 고 적어 둔 그 자리다 |
| `--plan-only` / `--resume`(두 단계 실행) | 중 | GPU 한 장으로 7B → VL 을 시간분할하는 경로. 지금은 CLI 만 |
| `prova login`(세션 재사용) | 중 | `--session` 을 UI 에서 고를 수 없다 |
| 확장프로그램 트리거 | 하 | 현재 탭 URL 을 `localhost:7007` 로 넘기는 얇은 것 (로드맵) |
| 실행 중단 | 하~중 | 한번 시작하면 끝날 때까지 기다려야 한다 |

> **화면을 고칠 때** — `app.js`·`app.css` 는 서버가 `no-store` 로 주므로 새로고침이면
> 바로 반영된다. 색을 새로 만들지 않는다 — `theme.py` 가 유일한 출처이고, 여기서
> 만들면 리포트와 두 벌로 갈라진다.

---

## 5-2. 테스트용 기획서와 웹사이트 (픽스처)

Prova 를 시험하려면 **기획서와 그 기획서대로/어긋나게 만든 웹앱이 쌍으로** 있어야
한다. 계획서 4-2 의 '설계 문서 데이터셋 구성' 이 이것이다.

한 화면을 더할 때 만드는 것은 넷이다.

| 만드는 것 | 어디에 |
|---|---|
| 화면기획서 md → PDF | `fixtures/specs/<이름>_spec.md` · `uv run python scripts/make_spec_pdf.py <경로>` |
| 정답(골든) — mock 백엔드가 읽는다 | `fixtures/specs/<이름>_spec.golden.json` |
| 웹앱 화면 (good / bad / icons) | `sut/templates/<이름>.html` + `sut/app.py` 의 라우트 |
| 관통 테스트 | `tests/test_<이름>_e2e.py` |

Figma 입력은 선택이다 — `fixtures/figma/<이름>.json` (컴포넌트 이름은 Input ·
Button · Select · Checkbox 넷만 인식한다).

### 2026-09-23 에 더한 것

| 화면 | 새로 밟는 것 | 심은 결함 | 실측 |
|---|---|---|---|
| 문의하기 `contact` | **`pattern`(정규식) 규칙** — 기존은 전부 `format: email` 이었다 | H1 연락처 형식 · H2 문의 내용 최소 길이 · H3 접수 문구 | good 17/17 · bad 13P/4F |
| 비밀번호 변경 `change_password` | **`require_lowercase`·`require_digit`** — 쓰는 기획서가 없었다 | I1 소문자 · I2 숫자 · I3 확인 일치 | good 13/13 · bad 10P/3F |
| 배송지 등록 `shipping` | **선택 항목**(필수가 아닌 요소) — 기존 요소는 전부 필수였다 | J1 우편번호 숫자 · J2 주소 길이 · J3 저장 문구 | good 14/14 · bad 10P/4F |

셋 다 오탐 0건이다.

**쓰이지 않던 규칙을 일부러 골랐다.** `rule_expander` 가 지원하는데 아무 기획서도
쓰지 않는 규칙은 동작한다는 증거가 없다 — 이 저장소가 반복해서 치른 값이다
(있는 것과 동작하는 것은 다르다).

### `icons` 변형 — 2차 경로(VLM)를 재는 대상

다섯 화면(`login`·`search`·`contact`·`change_password`·`shipping`)에 있다.
**검증 로직은 good 과 완전히 같고** 화면의 접근성 이름이 하나도 없다 — 라벨은
아이콘, 버튼은 아이콘, placeholder 는 비운다.

placeholder 를 비우는 것이 중요하다. 살려 두면 재는 것이 '아이콘을 찾는 능력' 이
아니라 'placeholder 로 찾는 능력' 이 된다.

1차 경로만으로 **64건이 전부 실패**하고 대부분이 `요소 미탐지` 다. 2차 경로가
살려내야 할 항목이 그만큼이다. 기존 `nolabel` 변형은 검색 버튼 **하나**만
아이콘이라 채점할 항목이 적었다 — 실제로 selector vs VLM 비교에서 1차가 놓친 2건이
전부 그 버튼 하나에 몰려 있었다(모양이 하나뿐이면 그 하나만 재는 것이다).

> **기획서에는 아이콘을 적지 않는다.** 기획서가 '로그인' 이라고 쓰고 구현만
> 아이콘일 때만 1차가 막히고 2차가 할 일이 생긴다. 둘 다 아이콘이면 시험이
> 성립하지 않는다.

### 걸렸던 것 — PDF 줄바꿈이 값 안에서 일어나면

안내 문구를 `010-0000-0000` 으로 두었더니 PDF 가 그 자리에서 줄을 바꿔 파서가
`010-0000-000 0` 으로 읽었다. 값 안에서 줄이 바뀌면 공백이 끼어든다.

픽스처는 긴 숫자열을 피해 넘어갔지만, **실물 기획서에서 다시 만날 자리다** —
로드맵 최우선 항목('실물 기획서로 S1 확인')이 찾으려던 종류의 어긋남이다.

---

## 6. 지금 환경 상태 (2026-09-23 확인)
