...
| 도구 | 위치 | 이 파이프라인에 없는 것 |
|---|---|---|
| auth/search 추출기 | `scripts/legacy/extract_auth_search_json.py` | **HWPX 파싱**, **EasyOCR** |
...
```bash
python -m pip install -r scripts/legacy/requirements.txt
```
...
```bash
python scripts/legacy/extract_auth_search_json.py sample.pdf --ollama-model MODEL_NAME
```
