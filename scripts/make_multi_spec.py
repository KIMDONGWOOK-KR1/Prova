...
SCREENS = ("login", "signup", "search")
OUT = SPEC_DIR / "multi_spec.md"
...
def assemble() -> str:
    ...
    parts = [(SPEC_DIR / "_multi_front.md").read_text(encoding="utf-8").rstrip() + "\n"]
    for stem in SCREENS:
        parts.append(screen_body(stem) + f"\n{PAGE_BREAK}\n")
    parts.append((SPEC_DIR / "_multi_flows.md").read_text(encoding="utf-8").rstrip() + "\n")
    return "\n".join(parts)
