...
## 실행이 필요 없다 — 그리고 대개 이 스크립트도 필요 없다

결과물은 이미 `docs/teaching/media/comparison.png` 에 커밋돼 있다. 이 스크립트는
그것을 **다시 만들 때만** 돌린다. 입력 `runs/real-good`·`runs/real-bad` 는
gitignore 대상이라 갓 클론한 환경에는 없다(팀장 머신에만 있다).

있을 때는 그 실행에 저장된 스텝 스크린샷을 읽어 합성한다. S4 가 매 스텝
스크린샷을 남기도록 만들어 둔 것이 여기서 쓰인다.
