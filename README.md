...
| 화면 | 대상 | 결과 |
|---|---|---|
| 로그인 | `good` — 기획서를 지킨 구현 | **8/8 PASS** (통과율 100%) |
| 로그인 | `bad` — 검증을 빠뜨린 구현 | **3 PASS / 5 FAIL** — 심은 결함 3종을 모두 지목, 오탐 0건 |
| 회원가입 | `good` | **15/15 PASS** |
| 회원가입 | `bad` | **11 PASS / 4 FAIL** — 심은 결함 4개를 정확히 지목, 오탐 0건 |
| 검색 | `good` | **8/8 PASS** |
| 검색 | `bad` | **4 PASS / 4 FAIL** — 심은 결함 3개를 지목(하나는 두 경로에서), 오탐 0건 |
| 통합 문서 (3화면+흐름 2) | `good` | **33/33 PASS** |
| 통합 문서 (3화면+흐름 2) | `bad` | **18 PASS / 15 FAIL** — 화면별 결과가 단일 문서와 동일, 오탐 0건 |
...
uv run pytest          # 테스트 355개 (vLLM 없으면 정확도 측정 54개는 자동 skip)
```
...
같은 물음("이 필드는 검증되는가, 그냥 쓰이는가?")이 구멍 하나를 더 찾았다.
