...
| 화면 | 대상 | 결과 |
|---|---|---|
| 로그인 | `good` — 기획서를 지킨 구현 | **9/9 PASS** (통과율 100%) |
| 로그인 | `bad` — 검증을 빠뜨린 구현 | **3 PASS / 6 FAIL** — 심은 결함 4종을 모두 지목, 오탐 0건 |
| 회원가입 | `good` | **16/16 PASS** |
| 회원가입 | `bad` | **12 PASS / 4 FAIL** — 심은 결함 4개를 정확히 지목, 오탐 0건 |
| 검색 | `good` | **9/9 PASS** |
| 검색 | `bad` | **5 PASS / 4 FAIL** — 심은 결함 3개를 지목(하나는 두 경로에서), 오탐 0건 |
| 통합 문서 (3화면+흐름 2) | `good` | **36/36 PASS** |
| 통합 문서 (3화면+흐름 2) | `bad` | **20 PASS / 16 FAIL** — 화면별 결과가 단일 문서와 동일, 오탐 0건 |
...
uv run pytest          # 테스트 374개 (vLLM 없으면 정확도 측정 54개는 자동 skip)
```
