...
| 문의하기 (`pattern` 규칙을 처음 쓰는 화면) | mock (2026-09-23) | `good` **17/17 PASS** · `bad` 13 PASS / 4 FAIL — 연락처 정규식(H1)·문의 내용 최소 길이(H2)·접수 문구(H3, 두 경로), 오탐 0건 |
| 비밀번호 변경 (`require_lowercase`·`require_digit` 를 처음 쓰는 화면) | mock (2026-09-23) | `good` **13/13 PASS** · `bad` 10 PASS / 3 FAIL — 한 요소의 규칙 다섯 중 소문자(I1)·숫자(I2)만 빠뜨렸고 그 둘만 지목, 오탐 0건 |
| 배송지 등록 (**선택 항목**이 처음 들어가는 화면) | mock (2026-09-23) | `good` **14/14 PASS** · `bad` 10 PASS / 4 FAIL — 우편번호 숫자(J1)·주소 길이(J2)·저장 문구(J3). **선택 항목에는 필수 위반 케이스를 만들지 않는다** |
| `icons` 변형 5화면 (로그인·검색·문의하기·비밀번호 변경·배송지 등록) | mock (2026-09-23) | **0 PASS / 64 FAIL** — 접근성 이름이 하나도 없어 1차 경로의 네 전략이 전부 막힌다. 대부분이 `요소 미탐지` 다 — **2차 경로(VLM)가 살려내야 할 항목이 그만큼**이고, `nolabel`(버튼 하나·9건)보다 훨씬 넓다 |
...
sut/                          테스트 대상 미니 웹앱 — 9화면(로그인·회원가입·검색·비밀번호 찾기·상품등록·주문조회·문의하기·비밀번호 변경·배송지 등록) good / bad + 도구를 시험하는 변형 9종
└ /icons/*                    라벨·버튼이 전부 아이콘이고 placeholder 가 없다 — 2차 경로(VLM)를 재는 대상 (5화면)
