...
S1 추출 정확도도 골든 데이터와 대조해 확인했다 — 세 화면 **45/45 통과**.
...
uv run pytest          # 테스트 246개 (vLLM 없으면 정확도 측정 45개는 자동 skip)
```
...
이제 남은 것 중 값이 큰 것은 **한 PDF에 여러 화면**이다. WITCHES 실물 기획서는 화면당
파일로 오지 않을 가능성이 높고, 그러면 `ScreenSpec`이 1화면 1객체라는 전제부터 갈린다.
화면 경계를 찾는 일이 새로 생기고, 그건 표 구조만으로는 안 풀린다.
