...
#: 2차 경로가 받아들일 기본 최소 신뢰도 — 이 값 아래면 보정을 포기한다.
#
# VLM 은 못 찾았을 때도 그럴듯한 좌표를 낸다. 그 좌표를 누르면 엉뚱한 곳을 눌러
# 놓고 케이스를 진행하게 되고, FAIL 이 구현 결함인지 잘못 누른 것인지 구분할 수
# 없다 — ground() 가 '정확히 1개만 확정한다' 를 계약으로 삼은 것과 같은 이유다.
# 설정(grounding.vlm_confidence_threshold)으로 덮어쓸 수 있다.
#
# 다만 2026-08-22 의 50개 채점에서 이 관문은 오탐 7개 중 **하나도** 거르지
# 못했다(전부 신뢰도 0.8 이상). 모델의 자기 신고는 관문이 되지 못한다 — 실제
# 방어선은 좌표 아래 요소를 확인하는 _require_actionable 이다(노트 13).
MIN_CONFIDENCE = 0.5
