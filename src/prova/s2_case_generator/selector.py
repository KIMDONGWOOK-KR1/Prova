...
def select_cases(
    ...
    selection = CaseSelection(
        ...
        excluded_titles={c.case_id: c.title for c in cases if c.case_id not in chosen},
...
def select_by_ids(
  ...
  실행 가능한 케이스: {available}"""
    ...
    return picked, CaseSelection(
        ...
        excluded_titles={c.case_id: c.title for c in cases if c.case_id not in wanted},
        reason=reason,
        approved=True,
        warnings=warnings,
