...
그래서 화면별 내역을 단일 문서 실행의 기대값과 대조한다.
    ...
    login   good 8/8    bad 3 PASS / 5 FAIL
    signup  good 15/15  bad 11 PASS / 4 FAIL
...
둘 다 **화면 하나만 보면 결함이 아니다.** 회원가입 화면의 케이스 15건은 입력 검증만
...
EXPECTED_BY_SCREEN = {
    "good": {"login": (8, 0), "signup": (15, 0), "search": (8, 0)},
    "bad": {"login": (3, 5), "signup": (11, 4), "search": (4, 4)},
}
...
class TestScreenSplit:
    ...
    def test_케이스_총계(self, good_run):
        """login 8 + signup 15 + search 8 + 흐름 2 = 33."""
        report, _ = good_run
...
class TestBadVariant:
    ...
    def test_흐름_실패를_화면_실패와_섞지_않는다(self, bad_run):
        ...
        report, _ = bad_run
        assert report.summary["by_screen"]["login"]["fail"] == 5
    ...
    def test_오탐이_없다(self, bad_run):
        """심은 결함: 로그인 3종(5건) + 회원가입 4종 + 검색 3종(4건)
        + E1(계정 미등록) + E2(잘못된 링크) = 15건."""
        report, _ = bad_run
        assert report.summary["fail"] == 15, "\n".join(
