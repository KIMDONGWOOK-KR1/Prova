...
둘 다 **화면 하나만 보면 결함이 아니다.** 회원가입 화면의 케이스 14건은 입력 검증만
...
class TestScreenSplit:
    ...
    def test_케이스_총계(self, good_run):
        """login 7 + signup 14 + search 8 + 흐름 2 = 31."""
        report, _ = good_run
...
class TestBadVariant:
    ...
    def test_오탐이_없다(self, bad_run):
        """심은 결함: 로그인 3종(4건) + 회원가입 3종 + 검색 3종(4건)
        + E1(계정 미등록) + E2(잘못된 링크) = 13건."""
        report, _ = bad_run
        assert report.summary["fail"] == 13, "\n".join(
