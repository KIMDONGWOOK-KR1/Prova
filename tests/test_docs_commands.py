...
## 이것으로 충분하지 않다 (2026-08-28)

`--reload` 는 프로세스를 둘로 만든다. 부모(감시자)가 죽으면 자식(일꾼)만 남아
옛 코드를 계속 서빙한다 — 명령을 옳게 쳤어도 그렇게 된다. 즉 **이 테스트는
문서가 옛 명령을 되살리는 것만 막고, 실제로 감시가 살아 있는지는 재지 못한다.**
그건 실행 시점의 사실이라 문서 검사로는 닿지 않는다. `/__build__` 도장과
`prova.sut_build` 가 그쪽을 맡는다 (티칭 노트 20 의 2026-08-28 추기).
